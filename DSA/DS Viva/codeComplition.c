...
void insertbegin() 									
{
    ...
    if (head==NULL && new!=NULL){
        head=new;
            count++// where new is the new node and hence element is inserted 
    }
    else{
        if (new==NULL){
            printf("No new NEW NODE");
        }
        else{
        new->next=head;   // pointing to head variable 
        head=new;
            count++} // changing the link of head from the old one to new one 
    }
